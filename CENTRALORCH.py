...
def finish_pool_node(state: OrchestratorState) ->OrchestratorState:
    ...
    if "call_resolve_human_intervention" not in completed:
        state["completed_actions"] = completed + ["call_resolve_human_intervention"]
    return state
...
- If the raw transcript is obtained ,before cleanup call the resolve human intervention function and also choose this only one and after that bypass it to the next action. This is not the implicit remember to choose this and make sure you see this step in completed to proceed to next step: choose "call_resolve_human_intervention"
